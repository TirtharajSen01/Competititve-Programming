...
void solve()
{
    ...
    maxD = -1;
    dfs(1 , 0); //can choose any node but problem is that the node must be present so if there are only 1 node so taking any other node dont give results take 1 for being safe 
